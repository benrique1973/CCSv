...
namespace SGPTWpf.SGPtWpf.ViewModel.Encargos.Supervision.Agenda
{

    public sealed class AgendaControllerViewModel : ViewModeloBase
    {
        ...
        private ObservableCollection<CedulaAgendaModelo> _listaDetalleEntidad;

        public ObservableCollection<CedulaAgendaModelo> listaDetalleEntidad
        {
            get
            {
        ...
        #region ViewModel Properties : listaTareas

        public const string listaTareasPropertyName = "listaTareas";

        private ObservableCollection<CedulaAgendaModelo> _listaTareas;

        public ObservableCollection<CedulaAgendaModelo> listaTareas
        {
            get
            {
                return _listaTareas;
            ...
            set
            {
                if (_listaTareas == value) return;

                _listaTareas = value;

                RaisePropertyChanged(listaTareasPropertyName);
            }
        }

        #endregion

        #region ViewModel Property : selectedAgendaModelo

        public const string selectedAgendaModeloPropertyName = "selectedAgendaModelo";

        private CedulaAgendaModelo _selectedAgendaModelo;

        public CedulaAgendaModelo selectedAgendaModelo
        {
            get
            {
                return _selectedAgendaModelo;
            }

            set
            {
                if (_selectedAgendaModelo == value) return;

                _selectedAgendaModelo = value;

                // Update bindings, no broadcast
                RaisePropertyChanged(selectedAgendaModeloPropertyName);
            }
        }

        #endregion

        #region ViewModel Properties : listaEmpleados
        ...
        #region ViewModel Property : selectedUsuarioModelo

        public const string selectedUsuarioModeloPropertyName = "selectedUsuarioModelo";

        private UsuarioModelo _selectedUsuarioModelo;

        public UsuarioModelo selectedUsuarioModelo
        {
            get
            {
                return _selectedUsuarioModelo;
            }

            set
            {
                if (_selectedUsuarioModelo == value) return;

                _selectedUsuarioModelo = value;

                // Update bindings, no broadcast
                RaisePropertyChanged(selectedUsuarioModeloPropertyName);
            }
        }
        ...
        private CedulaAgendaModelo _currentDetalleEntidad;

        public CedulaAgendaModelo currentDetalleEntidad
        {
            get
            {
        ...
        #region fechaaprobacionagenda

        public const string fechaaprobacionagendaPropertyName = "fechaaprobacionagenda";

        private DateTime _fechaaprobacionagenda = DateTime.Now;

        public DateTime fechaaprobacionagenda
        {
            get
            {
                return _fechaaprobacionagenda;
            }

            set
            {
                if (_fechaaprobacionagenda == value)
                {
                    return;
                }

                _fechaaprobacionagenda = value;
                RaisePropertyChanged(fechaaprobacionagendaPropertyName);
            }
        }
        ...
        public RelayCommand<CedulaAgendaModelo> SelectionTareaCommand { get; set; }
        ...
        public RelayCommand<DateTime> SelectedDateClienteCommand { get; set; }
        ...
        public RelayCommand<UsuarioModelo> SelectionEmpleadoCommand { get; set; }
        ...
        public AgendaControllerViewModel()
        {
        }

        public AgendaControllerViewModel(string origenLlamada)
        {
            ...
            _listaTareas = new ObservableCollection<CedulaAgendaModelo>();
            _listaDetalleEntidad = new ObservableCollection<CedulaAgendaModelo>();
            _selectedBalance = new BalanceModelo();
            _selectedAgendaModelo = new CedulaAgendaModelo();
            _listaEmpleados = new ObservableCollection<UsuarioModelo>();
            Errors = 0;
            ...
            _currentDetalleEntidad = new CedulaAgendaModelo();
            _eleccionTipoCedulaModelo = new TipoCedulaModelo();
            _selectedUsuarioModelo = new UsuarioModelo();
            ...
            {
                case "DocumentacionCedulaAgenda"://Llamada desde Documentacion/Carpetas
                    #region configuracion Documentacion
                    _tokenEnvio = "datosEncargoCedulaAgendaController";
                    _tokenRecepcion = "datosEncargoCedulaAgenda"; //Modificado
                    #endregion
                    break;

            }
            //Suscribiendo el mensaje
            Messenger.Default.Register<AgendaMsj>(this, tokenRecepcion, (datosMsj) => ControlDatosMsj(datosMsj));
            ...
            _fechaaprobacionagenda = new DateTime((DateTime.Now.Year), 1, 1);
        ...
        private async void ControlDatosMsj(AgendaMsj datosMsj)
        {
            //Errors = 0;
            //Asignacion de  entidades
            currentEncargo = datosMsj.encargoModelo;//Encargo en uso actual
            listaEmpleados= new ObservableCollection<UsuarioModelo>(UsuarioModelo.GetAllAsignados(currentEncargo.idencargo));
            ...
            if (currentDetalleEntidad != null && currentDetalleEntidad.idagenda!=0)
            {
                listaTareas = new ObservableCollection<CedulaAgendaModelo>(listaDetalleEntidad.Where(x => x.idagenda != currentDetalleEntidad.idagenda));
            }
            else
            {
                listaTareas = new ObservableCollection<CedulaAgendaModelo>(listaDetalleEntidad);
            }
            ...
            switch (datosMsj.opcionMenuCrud)
            {
                case 1://Crear 
                    ...
                    {
                        selectedAgendaModelo = listaTareas[0];
                    }
                    break;
                case 2://Editar, no sera activada
                    switch (currentDetalleEntidad.estadoprocesoagenda)
                    {
                        case "Asignada":
                            if (currentDetalleEntidad.idusuarioasigna == currentUsuario.idUsuario)
                            {
                            ...
                            break;
                        case "Respondida":
                            if (currentDetalleEntidad.idusuarioresponable == currentUsuario.idUsuario)
                            {
                                ...
                                visibilidadConsultar = Visibility.Collapsed;
                                visibilidadCopiar = Visibility.Collapsed;
                                visibilidadCuerpo = Visibility.Collapsed;
                            ...
                            break;
                        case "Terminada":
                            //await mensajeAutoCerrado("La tarea ya fue cerrada", "No puede editarla", 2);
                            break;
                    }
                    break;
                case 3://Consultar
            ...
            Messenger.Default.Unregister<AgendaMsj>(this, tokenRecepcion);
            finComando();
        ...
        private async void llenadoDatos(int comando)
        {
            if (listaTareas.Count() > 0)
            {
                visibilidadTareas = Visibility.Visible;
                CedulaAgendaModelo temporal = new CedulaAgendaModelo
                {
                    idCorrelativo = 0,
                    idagenda = 0,
                    encabezadoagenda = "Ninguna",
                };
                listaTareas.Insert(0, temporal);
            }

            if (currentDetalleEntidad.padreidagenda != null && currentDetalleEntidad.padreidagenda != 0)
            {

                if (listaTareas.Count() > 0)
                {
                    selectedAgendaModelo = listaTareas.Single(i => i.idagenda == currentDetalleEntidad.padreidagenda);
                }
            }
            else
            {
            ...
            if (currentDetalleEntidad.idusuarioresponable != null && currentDetalleEntidad.idusuarioresponable != 0)
            {
                selectedUsuarioModelo = listaEmpleados.Single(i => i.idUsuario == currentDetalleEntidad.idusuarioresponable);
            }
            else
            {
                selectedUsuarioModelo = listaEmpleados[0];
            }
            if (string.IsNullOrEmpty(currentDetalleEntidad.comentariorespuestaagenda))
            {
                visibilidadRespuesta = Visibility.Collapsed;
            }
            else
            ...
            if (comando != 1 || comando != 2 || comando != 3)
            {
                ...
                if (currentDetalleEntidad.fechaaprobacionagenda != null && currentDetalleEntidad.fechaaprobacionagenda != "")
                {
                    try
                    {
                        fechaaprobacionagenda = MetodosModelo.convertirFecha(currentDetalleEntidad.fechaaprobacionagenda);
                    }
                    catch (Exception)
                    {
                        //await dlg.ShowMessageAsync(this, "No se pudo convertir la fecha de inicio  de la auditoria", "");
                        await dlg.ShowMessageAsync(this, "No ha sido posible convertir la fecha", "");
                        fechaaprobacionagenda = new DateTime((DateTime.Now.Year), 1, 1);
                    }
                }
                else
                {
        ...
        {
            iniciarComando();
            if (nombreUnico(currentDetalleEntidad.encabezadoagenda, listaDetalleEntidad) == 0)
            {
                try
                {

                    //Se envian los detalles
                    switch (CedulaAgendaModelo.Insert(currentDetalleEntidad))
                    {
                        case 0://No se pudo insertar
                            finComando();
        ...
        public bool CanSave()
        {
            ...
            else
            {
                ...
                {
                    evaluar = Errors == 0 &&
                         !string.IsNullOrEmpty(currentDetalleEntidad.comentariorespuestaagenda)
                         && !string.IsNullOrWhiteSpace(currentDetalleEntidad.comentariorespuestaagenda);
                }
                else
                {
                    evaluar = Errors == 0;
        ...
        private void RegisterCommands()
        {
            ...
            SelectionTareaCommand = new RelayCommand<CedulaAgendaModelo>(entidad =>
            {
                if (entidad == null) return;
                if (entidad.idagenda != 0)
                {
                    currentDetalleEntidad.padreidagenda = entidad.idagenda;
                    currentDetalleEntidad.padreencabezadoagenda = entidad.encabezadoagenda;
                }
            });
            ...
            SelectionEmpleadoCommand = new RelayCommand<UsuarioModelo>(entidad =>
            {
                if (entidad == null) return;
                if (entidad.idUsuario != 0)
                {
                    currentDetalleEntidad.idusuarioresponable = entidad.idUsuario;
                    currentDetalleEntidad.usuariosuperviso = entidad.inicialesusuario;//Usuario  asignado
                }
            ...
            SelectedDateClienteCommand = new RelayCommand<DateTime>(entidad =>
            {
                if (entidad == null) return;

                if (finInicializacion)
                {
                    if (fechaaprobacionagenda != null)
                    {
                        currentDetalleEntidad.fechaaprobacionagenda = MetodosModelo.homologacionFecha(fechaaprobacionagenda.ToShortDateString());
                    }
                }
            });
        ...
        private bool CanNuevo()
        {
            ...
            else
            {
                ...
                //if (opcionMenu == 1)
                //{
                //    //if (!string.IsNullOrEmpty(currentDetalleEntidad.encabezadoagenda)
                //    //    && !string.IsNullOrEmpty(currentDetalleEntidad.descripcionagenda)
                //    //    && (currentDetalleEntidad.idusuarioasigna != null && currentDetalleEntidad.idusuarioasigna != 0)
                //    //    && (Errors == 0 || Errors == 1))
                //    //{
                //    //    return true;
                //    //}
        ...
        private bool CanModificar()
        {
            ...
            {
                evaluar = (Errors == 0)
                && (!string.IsNullOrWhiteSpace(currentEntidad.titulocedula));
                return evaluar;
            }
        }
            ...
            try
            {
                ...
                switch (opcionMenu)
                {
                    case 2: //Editar solo actualiza la fecha de evaluación
                        ...
                        {
                            case "Asignada":
                                if (nombreUnico(currentDetalleEntidad.encabezadoagenda, listaDetalleEntidad) == 1)
                                {
                                    resultadoModificar = CedulaAgendaModelo.UpdateModelo(currentDetalleEntidad, true);
                                }
                                else
                                {
                            ...
                            case "Respondida":
                                if (string.IsNullOrEmpty(currentDetalleEntidad.comentariorespuestaagenda))
                                {
                                    await mensajeAutoCerrado("Debe responder a la tarea", "Redacte su respuesta", 2);
                                    finComando();
                                }
                                else
                                {
                                    resultadoModificar = CedulaAgendaModelo.UpdateResponder(currentDetalleEntidad);
                                    currentDetalleEntidad.estadoprocesoagenda = "Respondida";
                                }
                                break;
                            case "Terminada":
                                //await mensajeAutoCerrado("La tarea ya fue cerrada", "No puede editarla", 2);
        ...
        public int nombreUnico(string nombre, ObservableCollection<CedulaAgendaModelo> listaBusqueda)
        {
            int numeroRegistros = 0;
            if (listaBusqueda != null || listaBusqueda.Count > 0)
            {
                string nombreBuscado = nombre.ToUpper().Trim();
                numeroRegistros = listaBusqueda.Where(j => j.encabezadoagenda.ToUpper().Trim() == nombreBuscado).Count();
                if (numeroRegistros == 1)
                {
                    return 1;
                }

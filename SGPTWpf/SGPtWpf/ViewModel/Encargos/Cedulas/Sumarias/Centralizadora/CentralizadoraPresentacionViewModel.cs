...
{
    public sealed class CentralizadoraPresentacionViewModel : ViewModeloBase
    {
        ...
        #region ViewModel Properties : fechabalancebalanceComparativo

        public const string fechabalancebalanceComparativoPropertyName = "fechabalancebalanceComparativo";

        private string _fechabalancebalanceComparativo;

        public string fechabalancebalanceComparativo
        {
            get
            {
                return _fechabalancebalanceComparativo;
            }

            set
            {
                if (_fechabalancebalanceComparativo == value)
                {
                    return;
                }

                _fechabalancebalanceComparativo = value;
                RaisePropertyChanged(fechabalancebalanceComparativoPropertyName);
            }
        }
        ...
        public CentralizadoraPresentacionViewModel()
        {

        }

        public CentralizadoraPresentacionViewModel(string origen)
        {
            #region configuracion general
            enviarMensajeInhabilitar();
            _currentEntidad = new CedulaModelo();
            _currentEntidadDetalle = new CedulaModelo();
            _lista = new ObservableCollection<CedulaModelo>();
            ...
            switch (origen)
            {
                case "CedulaCentralizadora":
                    _tokenEnvioCierre = "datosControllerEncargoCedulasSumariasCentralizadora";
                    _tokenRecepcion = "datosEncargoCedulasSumariasCentralizadora"; //Modificado
                    _fuenteLlamado = 1;
                    _encabezadoPantalla = "Cédula Resumen de Sumarias";
                    break;
                case "IndiceCedulaCentralizadora":
                    ...
                    break;
                case "ImprimirIndiceCedulaCentralizadora":
                    _tokenRecepcion = "datosPadreDetalleIndices";  //Modificado
            ...
            _fechabalancebalanceComparativo = string.Empty;
        ...
        private void ControlDetalleHerramientoCrudMensaje(CedulaMsj ElementoMsj)
        {
            ...
            if (currentEntidad.descripcionvisita == null || string.IsNullOrEmpty(currentEntidad.descripcionvisita))
            {
                currentEntidad.descripcionvisita = "Todas";
            }
            //lista = ElementoMsj.listaDetalle;
            //actualizarLista();
            lista = ElementoMsj.listaMaestroModelo;
            ...
            if (currentEntidad.fechabalancebalance != null)
            {
                ...
                if (fechaBalanceActual == null || string.IsNullOrEmpty(fechaBalanceActual))
                {
                    fechaBalanceActual = lista[0].fechabalancebalance;
                    currentEntidad.fechabalancebalance = lista[0].fechabalancebalance;
                }
            }
            if (currentEntidad.fechabalancebalanceComparativo != null)
            {
                fechabalancebalanceComparativo = currentEntidad.fechabalancebalanceComparativo;
                if (fechabalancebalanceComparativo  == null || string.IsNullOrEmpty(fechabalancebalanceComparativo ))
                {
                    fechabalancebalanceComparativo  = lista[0].fechabalancebalanceComparativo;
        ...
        private void actualizarLista()
        {
            try
            {
                ...
                //lista = new ObservableCollection<CedulaModelo>(CedulaModelo.GetAllEdicion(currentEncargo, currentEntidad.idcedula));

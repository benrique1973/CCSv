...
namespace SGPTWpf.SGPtWpf.ViewModel.Encargos.Cedulas.CumplimientoLegal
{

    public sealed class CumplimientoLegalViewModel : ViewModeloBase
    {
        ...
        public CumplimientoLegalViewModel()//Caso Encargo/PlanIndice
        {
        }

        //Llamado desde documentacion
        public CumplimientoLegalViewModel(string origen)//Documentacion/Carpetas
        {
            ...
            switch (origen)
            {
                case "MenuCedulas":
                    ...
                    _tokenRecepcionPadre = "Ajustes y reclasificaciones" + origen;//Permite captar los mensajes del  menú planificacion, corresponde a Indices

                    _tokenRecepcionCierrePreView = "CierreEncargoCedulasAjustes";//Sirve tanto para los programas en vista previa como para el controllador;

                    _tokenEnvioDatosAHijo = "datosEncargoCedulasAjustes";//Para control de los datos que  remite programas a sub-ventanas

                    _tokenRecepcionHijo = "datosEncargoCedulasAjustesController";

                    _tokenRecepcionSubMenu = "datosEncargoCedulasAjustesControllerRegreso";
        ...
        public void Nuevo()
        {
            ...
            ECMainModel.TypeName = "TipoCarpetaModeloCrearview";
            #endregion
        ...
        public async void Editar()
        {
            ...
            if (lista.Where(x => x.IsSelected).Count() == 1)
            {
                if (!(currentEntidad == null))
                {
                    iniciarComando();
                    comando = 2;
                    ECMainModel.TypeName = "TipoCarpetaModeloEditarView";
        ...
        public async void Consultar()
        {
            ...
            if (lista.Where(x => x.IsSelected).Count() == 1)
            {
                if (!(currentEntidad == null))
                {
                    iniciarComando();
                    comando = 3;
                    ECMainModel.TypeName = "TipoCarpetaModeloConsultarView";
                    enviarMensaje(); //Debe ir antes, porque evalua si la ventana es cero para enviarse
        ...
        private async void aprobarPapel()
        {
            if (!(currentEntidad == null))
            {
                ...
                ECMainModel.TypeName = "IndiceModeloAprobarView";
        ...
        {
            {
                if (!(currentEntidad == null))
                {
                    ...
                    ECMainModel.TypeName = "IndiceModeloSupervisarView";
                    #region Configuracion
                    currentEntidad.usuarioModelo = usuarioModelo;
                    #endregion
                    enviarMensaje();  //Debe ir antes, porque evalua si la ventana es cero para enviarse
            ...
            if (!(currentEntidad == null))
            {
                ...
                if (estadoDocumento == 1) //Hay cero procedimientos diferentes a iniciados
                {
                    if (currentEntidad.IndiceItemsTotales == 100)
                    {
                        ...
                        ECMainModel.TypeName = "IndiceModeloCerrarView";
                        #region Configuracion
                        currentEntidad.usuarioModelo = usuarioModelo;
                        #endregion
                        enviarMensaje();  //Debe ir antes, porque evalua si la ventana es cero para enviarse
            ...
            switch (TipoCarpetaModelo.evaluarImportacion(currentEncargo.idencargo))
            {
                ...
                default:
                    ...
                    if (result == MessageDialogResult.Affirmative)
                    {
                        ...
                        ECMainModel.TypeName = "TipoCarpetaModeloImportarView";

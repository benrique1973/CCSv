...
namespace SGPTWpf.SGPtWpf.ViewModel.Encargos.Cedulas.Notas
{
    ...
    public sealed class NotasControllerViewModel : ViewModeloBase
    {
        ...
        private ObservableCollection<CedulaNotasModelo> _listaDetalleEntidad;

        public ObservableCollection<CedulaNotasModelo> listaDetalleEntidad
        {
            get
            {
        ...
        private CedulaNotasModelo _currentDetalleEntidad;

        public CedulaNotasModelo currentDetalleEntidad
        {
            get
            {
        ...
        public NotasControllerViewModel()
        {
        }

        public NotasControllerViewModel(string origenLlamada)
        {
            ...
            _listaDetalleEntidad = new ObservableCollection<CedulaNotasModelo>();
            ...
            _currentDetalleEntidad = new CedulaNotasModelo();
            _eleccionTipoCedulaModelo = new TipoCedulaModelo();
            ...
            {
                case "DocumentacionCedulaNotas"://Llamada desde Documentacion/Carpetas
                    #region configuracion Documentacion
                    _tokenEnvio = "datosEncargoCedulasNotasController";
                    _tokenRecepcion = "datosEncargoCedulasNotas"; //Modificado
                    #endregion
                    break;
                case "DocumentacionCedulasNotasResumen"://Llamada desde Documentacion/Carpetas
                    #region configuracion Documentacion
        ...
        private async void ControlDatosMsj(NotasMsj datosMsj)
        {
            ...
            Messenger.Default.Unregister<NotasMsj>(this, tokenRecepcion);
            finComando();
        ...
        {
            iniciarComando();
            if (nombreUnico(currentDetalleEntidad.descripcionnotaspt, listaDetalleEntidad) == 0)
            {
                try
                {

                    //Se envian los detalles
                    switch (CedulaNotasModelo.Insert(currentDetalleEntidad))
                    {
                        case 0://No se pudo insertar
                            finComando();
            ...
            try
            {
                ...
                switch (opcionMenu)
                {
                    case 2: //Editar solo actualiza la fecha de evaluación
                        if (nombreUnico(currentDetalleEntidad.descripcionnotaspt, listaDetalleEntidad) == 1)
                        {
                            resultadoModificar = CedulaNotasModelo.UpdateModelo(currentDetalleEntidad, true);
                        }
                        else
                        {
        ...
        public int nombreUnico(string nombre, ObservableCollection<CedulaNotasModelo> listaBusqueda)
        {
            int numeroRegistros = 0;
            if (listaBusqueda != null || listaBusqueda.Count > 0)
            {
                string nombreBuscado = nombre.ToUpper().Trim();
                numeroRegistros = listaBusqueda.Where(j => j.descripcionnotaspt.ToUpper().Trim() == nombreBuscado).Count();
                if (numeroRegistros == 1)
                {
                    return 1;
                }

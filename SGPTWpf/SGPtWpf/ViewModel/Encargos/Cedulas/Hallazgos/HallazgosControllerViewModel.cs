...
namespace SGPTWpf.SGPtWpf.ViewModel.Encargos.Cedulas.Hallazgos
{
    ...
    public sealed class HallazgosControllerViewModel : ViewModeloBase
    {
        ...
        private ObservableCollection<CedulaHallazgosModelo> _listaDetalleEntidad;

        public ObservableCollection<CedulaHallazgosModelo> listaDetalleEntidad
        {
            get
            {
        ...
        private CedulaHallazgosModelo _currentDetalleEntidad;

        public CedulaHallazgosModelo currentDetalleEntidad
        {
            get
            {
        ...
        #region fecharespclientehallazgo

        public const string fecharespclientehallazgoPropertyName = "fecharespclientehallazgo";

        private DateTime _fecharespclientehallazgo = DateTime.Now;

        public DateTime fecharespclientehallazgo
        {
            get
            {
                return _fecharespclientehallazgo;
            }

            set
            {
                if (_fecharespclientehallazgo == value)
                {
                    return;
                }

                _fecharespclientehallazgo = value;
                RaisePropertyChanged(fecharespclientehallazgoPropertyName);
            }
        }
        ...
        public HallazgosControllerViewModel()
        {
        }

        public HallazgosControllerViewModel(string origenLlamada)
        {
            ...
            _listaDetalleEntidad = new ObservableCollection<CedulaHallazgosModelo>();
            ...
            _currentDetalleEntidad = new CedulaHallazgosModelo();
            _eleccionTipoCedulaModelo = new TipoCedulaModelo();
            ...
            switch (origenLlamada)
            {
                case "DocumentacionCedulasHallazgos"://Llamada desde Documentacion/Carpetas
                    ...
                    _tokenEnvio = "datosEncargoCedulasHallazgosController";
                    _tokenRecepcion = "datosEncargoCedulasHallazgos"; //Modificado
                    #endregion
                    break;

                case "DocumentacionCedulasHallazgosResumen"://Llamada desde Documentacion/Carpetas
                    #region configuracion Documentacion
                    _tokenEnvio = "datosEncargoCedulasHallazgosControllerMenuPrincipal";
                    _tokenRecepcion = "datosEncargoCedulasHallazgosMenuPrincipal"; //Modificado
                    #endregion
                    break;
            }
            //Suscribiendo el mensaje
            Messenger.Default.Register<HallazgosMsj>(this, tokenRecepcion, (datosMsj) => ControlDatosMsj(datosMsj));
            ...
            _fecharespclientehallazgo= new DateTime((DateTime.Now.Year), 1, 1);
        ...
        private async void ControlDatosMsj(HallazgosMsj datosMsj)
        {
            ...
            Messenger.Default.Unregister<HallazgosMsj>(this, tokenRecepcion);
            finComando();
        ...
        private async void llenadoDatos(int comando)
        {
            if (comando != 1 || comando != 2 || comando != 3)
            {
                ...
                {
                    if (currentDetalleEntidad.fecharespclientehallazgo != null && currentDetalleEntidad.fecharespclientehallazgo != "")
                    {
                        try
                        {
                            fecharespclientehallazgo = MetodosModelo.convertirFecha(currentDetalleEntidad.fecharespclientehallazgo);
                        }
                        catch (Exception)
                        {
                            //await dlg.ShowMessageAsync(this, "No se pudo convertir la fecha de inicio  de la auditoria", "");
                            await dlg.ShowMessageAsync(this, "No ha sido posible convertir la fecha", "");
                            fecharespclientehallazgo = new DateTime((DateTime.Now.Year), 1, 1);
                        }
                    }
                    else
                    {
        ...
        {
            iniciarComando();
            if (nombreUnico(currentDetalleEntidad.titulohallazgo, listaDetalleEntidad) == 0)
            {
                try
                {

                    //Se envian los detalles
                    switch (CedulaHallazgosModelo.Insert(currentDetalleEntidad))
                    {
                        case 0://No se pudo insertar
                            finComando();
        ...
        private void RegisterCommands()
        {
            ...
            SelectedDateClienteCommand = new RelayCommand<DateTime>(entidad =>
            {
                ...
                if (finInicializacion)
                {
                    if (fecharespclientehallazgo != null)
                    {
                        currentDetalleEntidad.fecharespclientehallazgo = MetodosModelo.homologacionFecha(fecharespclientehallazgo.ToShortDateString());
                    }
                }
            });
            ...
            try
            {
                ...
                switch (opcionMenu)
                {
                    case 2: //Editar solo actualiza la fecha de evaluación
                        if (nombreUnico(currentDetalleEntidad.titulohallazgo, listaDetalleEntidad) == 1)
                        {
                            resultadoModificar = CedulaHallazgosModelo.UpdateModelo(currentDetalleEntidad, true);
                        }
                        else
                        {
        ...
        public int nombreUnico(string nombre, ObservableCollection<CedulaHallazgosModelo> listaBusqueda)
        {
            int numeroRegistros = 0;
            if (listaBusqueda != null || listaBusqueda.Count > 0)
            {
                string nombreBuscado = nombre.ToUpper().Trim();
                numeroRegistros = listaBusqueda.Where(j => j.titulohallazgo.ToUpper().Trim() == nombreBuscado).Count();
                if (numeroRegistros == 1)
                {
                    return 1;
                }

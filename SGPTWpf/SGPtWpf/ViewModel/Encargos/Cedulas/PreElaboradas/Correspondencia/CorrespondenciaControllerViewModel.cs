...
namespace SGPTWpf.SGPtWpf.ViewModel.Encargos.Cedulas.PreElaboradas.Correspondencia
{
    ...
    public sealed class CorrespondenciaControllerViewModel : ViewModeloBase
    {
        ...
        public CorrespondenciaControllerViewModel()
        {
        }

        public CorrespondenciaControllerViewModel(string origenLlamada)
        {
            ...
            {
                case "DocumentacionCedulasCorrespondencias"://Llamada desde Documentacion/Carpetas
                    #region configuracion Documentacion
                    _tokenEnvio = "datosEncargoCedulasCorrespondenciaController";
                    _tokenRecepcion = "datosEncargoCedulasCorrespondencia"; //Modificado
                    #endregion
                    break;

            }
            //Suscribiendo el mensaje
            Messenger.Default.Register<CorrespondenciaMsj>(this, tokenRecepcion, (datosMsj) => ControlDatosMsj(datosMsj));
        ...
        private async void ControlDatosMsj(CorrespondenciaMsj datosMsj)
        {
            ...
            Messenger.Default.Unregister<CorrespondenciaMsj>(this, tokenRecepcion);
            finComando();
        ...
        {
            if (nombreUnico(currentEntidad.titulocedula, listacurrentEntidad) == 0)
            {
                try
                {
            ...
            try
            {
                ...
                switch (opcionMenu)
                {
                    case 2: //Editar solo actualiza la fecha de evaluación
                        if (nombreUnico(currentEntidad.titulocedula, listacurrentEntidad) == 1)
                        {

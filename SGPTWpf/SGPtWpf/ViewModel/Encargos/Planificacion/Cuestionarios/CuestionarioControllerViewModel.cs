...
{
    public sealed class CuestionarioControllerViewModel : ViewModeloBase
    {
        ...
        private DetalleCuestionarioModelo _currentEntidadProgramaDetalle;

        public DetalleCuestionarioModelo currentEntidadProgramaDetalle
        {
            get
            {
        ...
        public CuestionarioControllerViewModel(string origen)//Documentacion/Carpetas
        {
            ...
            _resultadoProceso = 0;
            _tokenEnvioController = "datosControllerCuestionario";//Modificado
            _tokenEnvioControllerDetalle = "datosCuestionarioDetalle";//Modificado
            _tokenRecepcionVista = "listaDetalleProgramaVista";
            _fuenteCierre = 0;
            _detalle = new MenuEncargoCuestionario();//Es generico el view display es cualquier string
            _maxnombreprograma = 500;
            _opcionMenu = 0;
            Errors = 0;
            _tokenRecepcionProgramas = "datosEncargosCuestionario";//Modificado
            ...
            _tokenHabilitarVentanaPadreRecepcion = "HabilitarVentanaPadreEncargoPlanEdicionCuestionario"; 
            ...
            _listaDetallePrograma = new ObservableCollection<DetalleCuestionarioModelo>();
        ...
        public CuestionarioControllerViewModel(int comando)
        {
            configuracionDialogo = new MetroDialogSettings()
            {
                AnimateShow = false,
            ...
            _resultadoProceso = 0;
            _tokenEnvioController = "datosControllerCuestionario";//Modificado
            _tokenEnvioControllerDetalle = "datosCuestionarioDetalle";
        ...
        {   //Permite manejar cambios en la lista realizados por drag and drop
            listaDetallePrograma = DetalleListaMensaje.listaElementosCuestionario;
        ...
        private async void ControlProgramaDatosMsj(ProgramaDatosMsj programaDatosMsj)
        {
            ...
            switch (programaDatosMsj.opcionMenuCrud)
            {
                case 1://Crear 
                    ...
                    currentEntidadPrograma.tipoHerramientaModeloPrograma = listaTipoHerramienta[1]; //Es un cuestionario
                    ...
                    SelectedTipoHerramientaModelo = listaTipoHerramienta[1];//Es un cuestionario
                    encabezadoPantalla = "Introduzca los datos para el cuestionario";
                    nombreprogramaVista = "Ingrese el nombre del cuestionario";
        ...
        public async void guardarLista()
        {
            if (listaDetallePrograma.Count > 0 && listaDetallePrograma.Where(x => x.guardadoBase == false).Count() >0)
            {
                ...
                foreach (DetalleCuestionarioModelo item in listatemporal)
                {
            ...
            try
            {
                switch (ProgramaModelo.InsertByCreacion(currentEntidadPrograma))
                {
                    ...
                    case 1://Se inserto con éxito
                        ...
                        if (DetalleCuestionarioModelo.Insert(currentEntidadProgramaDetalle, true, currentEncargo.idencargo))
                        {
                            var controller = await dlg.ShowProgressAsync(this, "Registro insertado con éxito", "Este mensaje desaparecerá en 2 segundos", settings: configuracionDialogo);
                            controller.SetIndeterminate();
                            await TaskEx.Delay(1000);
                ...
                for (int k = 0; k < listaPlantillas.Count; k++)
                {
                    if (listaPlantillas[k].seleccionHerramienta)
                    {
                        ...
                        if (validarPlantilla(listaPlantillas[k]))
                        {
                            ...
                            if (continuar == 0)
                            {
                                var controller = await dlg.ShowProgressAsync(this, "La plantilla denominada " + listaPlantillas[k].nombreHerramienta + " fue importada con  éxito", "Este mensaje desaparecerá en segundos", settings: configuracionDialogo);
                                controller.SetIndeterminate();
                                await TaskEx.Delay(1000);
                                await controller.CloseAsync();
                            }
                            ...
                            {
                                var controller = await dlg.ShowProgressAsync(this, "La plantilla denominada " + listaPlantillas[k].nombreHerramienta + " no pudo ser importada con  éxito", "Este mensaje desaparecerá en segundos", settings: configuracionDialogo);
                                controller.SetIndeterminate();
                                await TaskEx.Delay(1000);
                                await controller.CloseAsync();
                        ...
                        {
                            var controller = await dlg.ShowProgressAsync(this, "Ya existe un programa con el nombre " + listaPlantillas[k].nombreHerramienta + " por lo que no  se importará", "Este mensaje desaparecerá en 1 segundos", settings: configuracionDialogo);
                            controller.SetIndeterminate();
                            await TaskEx.Delay(1000);
                            await controller.CloseAsync();
                        }
        ...
        public int ImportarPlantillasByItem(ProgramaModelo entidad, HerramientasModelo herramientaPlantilla)
        {
            ...
            switch (ProgramaModelo.InsertByCreacion(entidad))
            {
                ...
                case 1://Caso normal se inserto el principal y el auxiliar, se procede  a insertar los dependientes
                    ...
                    ObservableCollection<detallecuestionario> listaDestino = new ObservableCollection<detallecuestionario>();
                    temporal = null;
                    for (int p = 0; p < listaDetalle.Count; p++)
                    {
                        ...
                        temporal = DetalleCuestionarioModelo.DetalleCuestionarioConversion(currentEntidadProgramaDetalle, listaDetalle[p]);
                    ...
                    if (resultado == 0)
                    {
                        ...
                        //listaDestino = new ObservableCollection<detallecuestionario>(DetalleCuestionarioModelo.GetAllCapaDatos(entidad.idprograma));
                        ...
                        foreach (detalleherramienta itemDetalle in listaDetalle)
                        {
                            if (itemDetalle.iddhprincipaldh != null)
                            {
                                ...
                                temporal = listaDestino.Single(j => j.ordendc == itemDetalle.ordendh);
                                padre= listaDetalle.Single(j => j.iddh == itemDetalle.iddhprincipaldh);
                                temporal.idpadredc= listaDestino.Single(j => j.ordendc == padre.ordendh).iddc;
        ...
        public int ImportarProgramasByItemForEncargos(ProgramaModelo entidad, ProgramaModelo programaOrigen)
        {
            ...
            switch (ProgramaModelo.InsertByCreacion(entidad))
            {
                ...
                case 1://Caso normal se inserto el principal y el auxiliar, se procede  a insertar los dependientes
                       //Conseguir el listado de detalles de la  herramienta
                    ObservableCollection<DetalleCuestionarioModelo> listaDetalle = new ObservableCollection<DetalleCuestionarioModelo>(DetalleCuestionarioModelo.GetAll(programaOrigen.idprograma,currentEncargo.idencargo));
                    ObservableCollection<DetalleCuestionarioModelo> listaDestino = new ObservableCollection<DetalleCuestionarioModelo>();
                    currentEntidadProgramaDetalle.idprogramadp = entidad.idprograma;//Aplica para todos
                    i = 0;
                    temporal = null;
                    for (int p = 0; p < listaDetalle.Count; p++)
                    {
                        ...
                        temporal = new DetalleCuestionarioModelo(currentEntidadProgramaDetalle, listaDetalle[p]);
                        listaDestino.Add(temporal);
                        switch (DetalleCuestionarioModelo.InsertbyImportacion(listaDestino[i], currentEncargo.idencargo))
                        {
                            case 0:
                                resultado = 2;//No pudo insertar un detalle
                    ...
                    if (resultado == 0)
                    {
                        foreach (DetalleCuestionarioModelo itemDetalle in listaDestino)
                        {
                            if (itemDetalle.idpadredp != null && itemDetalle.guardadoBase)
                            {
                                ...
                                if (DetalleCuestionarioModelo.UpdateModeloByImportacion(itemDetalle))
                                {
                                    //codigoError = 7;
                                }
                                else

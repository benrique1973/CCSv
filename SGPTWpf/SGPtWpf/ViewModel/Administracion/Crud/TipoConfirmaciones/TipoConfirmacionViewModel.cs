...
using SGPTWpf.Messages.TipoConfirmaciones;
using SGPTWpf.Model;
...
namespace SGPTWpf.ViewModel.Crud.TipoConfirmacion
{
    /// <summary>
    ...
    /// </para>
    /// </summary>
    public sealed class TipoConfirmacionViewModel : ViewModeloBase
    {
        ...
        private ObservableCollection<TipoConfirmacionModelo> _Lista;

        public ObservableCollection<TipoConfirmacionModelo> Lista
        {
            get
            {
                return _Lista;
        ...
        /// <summary>
        /// The <see cref="descripciontconfirmacion" /> property's name.
        /// </summary>
        ...
        private TipoConfirmacionModelo _currentEntidad;

        public TipoConfirmacionModelo currentEntidad
        {
            get
            {
        ...
        public RelayCommand<TipoConfirmacionModelo> SelectionChangedCommand { get; set; }
        ...
        // Metodo adicionado
        public TipoConfirmacionViewModel(string origen)//Documentacion/Carpetas
        {
            ...
            _tokenRecepcionHijo = "";
            _tokenRecepcionPadre = "Tipos de confirmaciones" + "CatalogosViewModel";//Permite captar los mensajes del  menú planificacion
            ...
            Lista = new ObservableCollection<TipoConfirmacionModelo>(TipoConfirmacionModelo.GetAll());
            RegisterCommands();
        ...
        {
            //Se crea el mensaje
            TipoConfirmacionElementoMensaje elemento = new TipoConfirmacionElementoMensaje();
            if ((ventanas == 0))
            {
            ...
            Messenger.Default.Send<TipoConfirmacionElementoMensaje>(elemento);
        }
        public void enviarLista()
        {
            //Se crea el mensaje
            TipoConfirmacionListaMensaje listaEnviada = new TipoConfirmacionListaMensaje();
            if ((ventanas == 0))
            {
            ...
            Messenger.Default.Send<TipoConfirmacionListaMensaje>(listaEnviada);
        }
        #endregion
        ...
        public async void Nuevo()
        {
            if (ventanas == 0)
            {
                ...
                MainModel.TypeName = "TipoConfirmacionCrearView";
                currentEntidad = new TipoConfirmacionModelo();
                enviarElemento();
                enviarLista();
                enviarMensaje();
                comando = 1;
        ...
        public async void Editar()
        {
            if (!(currentEntidad == null))
            {
                if (ventanas == 0)
                {
                    ...
                    MainModel.TypeName = "TipoConfirmacionEditarView";
        ...
        public async void Consultar()
        {
            if (!(currentEntidad == null))
            {
                if (activarVentanaConsulta)
                {
                    if (ventanas == 0)
                    {
                        MainModel.TypeName = "TipoConfirmacionConsultarView";
                        enviarElemento();
                        enviarMensaje();
                        comando = 3;
        ...
        public async void Borrar()
        {
            if (!(currentEntidad == null))
            {
                if (ventanas == 0)
                {
                    ...
                    if (TipoConfirmacionModelo.DeleteBorradoLogico(currentEntidad.id, true))
                    {
        ...
        public void Actualizar(ObservableCollection<TipoConfirmacionModelo> listaEntidad)
        {
            Lista = listaEntidad;
        }
        ...
        private void RegisterCommands()
        {
            ...
            SelectionChangedCommand = new RelayCommand<TipoConfirmacionModelo>(entidad =>
            {
        ...
        private void actualizarLista()
        {
            ...
            catch (Exception e)
            {
                if (e.Source != null)
                {
                    ...
                    Lista = new ObservableCollection<TipoConfirmacionModelo>();
                }
            }
        }

...
namespace SGPTWpf.ViewModel.Crud.Clasificacion
{
    ...
    /// </para>
    /// </summary>
    public sealed class ClasificacionControllerViewModel : ViewModeloBase
    {
        ...
        private ObservableCollection<ClasificacionModelo> _ListaEntidad;

        public ObservableCollection<ClasificacionModelo> ListaEntidad
        {
            get
            {
        ...
        public ClasificacionControllerViewModel()
        {
            ...
            Messenger.Default.Register<ClasificacionElementoMensaje>(this, (controlElementoMensaje) => ControlElementoMensaje(controlElementoMensaje));
            Messenger.Default.Register<ClasificacionListaMensaje>(this, (controlListaMensaje) => ControlListaMensaje(controlListaMensaje));
            RegisterCommands();
        ...
        private void ControlElementoMensaje(ClasificacionElementoMensaje controlElementoMensaje)
        {
            currentEntidad = controlElementoMensaje.elementoMensaje; ;
        }

        private void ControlListaMensaje(ClasificacionListaMensaje controlListaMensaje)
        {
        ...
        {
            if (!(ClasificacionModelo.FindTexto(currentEntidad.descripcion)))
        {
            if (!string.IsNullOrEmpty(currentEntidad.descripcion))
            {
                ...
                if (currentEntidad.descripcion.Length <= maxDescripcion)
                {
                    ...
                    {
                        //currentEntidad.id = ClasificacionModelo.IdAsignar();
                        //if (!ClasificacionModelo.FindClaseCuentaPK(currentEntidad.idclasificacion))
                        //{
                        if (ClasificacionModelo.Insert(currentEntidad, true))
                        {
                    ...
                    else
                    {
                        ...
                        {
                            {
                                if (ClasificacionModelo.UpdateModelo(currentEntidad, true))
                                {
                                        var controller = await dlg.ShowProgressAsync(this, "Registro insertado con éxito", "Este mensaje desaparecerá en 2 segundos", settings: configuracionDialogo);
                                        controller.SetIndeterminate();
                                        await TaskEx.Delay(1000);

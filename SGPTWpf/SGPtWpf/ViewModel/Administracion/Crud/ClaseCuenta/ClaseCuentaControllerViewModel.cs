...
namespace SGPTWpf.ViewModel.Crud.ClaseCuenta
{
    public sealed class ClaseCuentaControllerViewModel : ViewModeloBase
    {
        ...
        private ObservableCollection<ClaseCuentaModelo> _ListaEntidad;

        public ObservableCollection<ClaseCuentaModelo> ListaEntidad
        {
            get
            {
        ...
        #region Propiedades : descripcionccuentas

        public const string descripcionPropertyName = "descripcionccuentas";
        ...
        private ClaseCuentaModelo _currentEntidad;

        public ClaseCuentaModelo currentEntidad
        {
            get
            {
        ...
        public RelayCommand<ClaseCuentaModelo> SelectionChangedCommand { get; set; }
        ...
        public ClaseCuentaControllerViewModel()
        {
            ...
            Messenger.Default.Register<ClaseCuentaElementoMensaje>(this, (controlElementoMensaje) => ControlElementoMensaje(controlElementoMensaje));
            Messenger.Default.Register<ClaseCuentaListaMensaje>(this, (controlListaMensaje) => ControlListaMensaje(controlListaMensaje));
            RegisterCommands();
        ...
        private void ControlElementoMensaje(ClaseCuentaElementoMensaje controlElementoMensaje)
        {
            currentEntidad = controlElementoMensaje.elementoMensaje; ;
        }

        private void ControlListaMensaje(ClaseCuentaListaMensaje controlListaMensaje)
        {
        ...
        {
            if (!(ClaseCuentaModelo.FindTexto(currentEntidad.descripcionccuentas)))
        {
            if (!string.IsNullOrEmpty(currentEntidad.descripcionccuentas))
            {

                if (currentEntidad.descripcionccuentas.Length <= maxDescripcion)
                {
                    if ((currentEntidad.id == 0))
                    {
                        //currentEntidad.id = ClaseCuentaModelo.IdAsignar();
                        //if (!ClaseCuentaModelo.FindClaseCuentaPK(currentEntidad.idccuentas))
                        //{
                        if (ClaseCuentaModelo.Insert(currentEntidad, true))
                        {
                    ...
                    else
                    {
                        ...
                        {
                            {
                                if (ClaseCuentaModelo.UpdateModelo(currentEntidad, true))
                                {
        ...
        public bool CanSave()
        {
            ...
            {
                evaluar = !string.IsNullOrEmpty(currentEntidad.descripcionccuentas) &&
                           (currentEntidad.descripcionccuentas.Length <= maxDescripcion);
                return evaluar;
            }
            }
        ...
        private void RegisterCommands()
        {
            ...
            SelectionChangedCommand = new RelayCommand<ClaseCuentaModelo>(entidad =>
            {

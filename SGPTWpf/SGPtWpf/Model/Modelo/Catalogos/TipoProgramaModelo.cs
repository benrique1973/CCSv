...
{
    public class TipoProgramaModelo : UIBase
    {
        ...
        public static bool Insert(TipoProgramaModelo modelo, Boolean insertar)
        {
            bool result = false;
            if (!(modelo == null))
            {
                try
                {
                    using (_context = new SGPTEntidades())
                    {
                        if (!(sincronizar))
                        {
                            ...
                            "SELECT pg_catalog.setval(pg_get_serial_sequence('sgpt.tiposprograma', 'idtp'), (SELECT MAX(idtp) FROM sgpt.tiposprograma) + 1);");
                            sincronizar = true;
                        }                        //var items = from c in _context.maestrocatalogos select c.idmaestrocatalogo;
                        var tablaDestino = new tiposprograma
                        {
                            //idtp = modelo.id,
                            descripciontp = modelo.descripcion,
                            sistematp = false,
                            estadotp = "A"
                        };
                        _context.tiposprogramas.Add(tablaDestino);
                        _context.SaveChanges();
                        modelo.id = tablaDestino.idtp;
                        modelo.sistema = tablaDestino.sistematp;
                        modelo.estado = tablaDestino.estadotp;
                        result = true;
                    }
                }
                catch (Exception e)
        ...
        public static string Insert(TipoProgramaModelo modelo)
        {
            string result = "";
            if (!(modelo == null))
            {
                try
                {
                    using (_context = new SGPTEntidades())
                    {
                        ...
                        modelo.sistema = tablaDestino.sistematp;
                        modelo.estado = tablaDestino.estadotp;
                        result = tablaDestino.idtp.ToString();
                    }
                }
                catch (Exception e)
                {
        ...
        public static int IdAsignar()
        {
            ...
            try
            {
                using (_context = new SGPTEntidades())
                {
                    idtp = _context.tiposprogramas.Max(x => x.idtp) + 1;
                }
            }
            catch (Exception e)
            {
        ...
        public static TipoProgramaModelo GetRegistro(int id)
        {
            var entidadModelo = new TipoProgramaModelo();
            if (!(id == 0))
            {
                using (_context = new SGPTEntidades())
                {
                    tiposprograma entidad = _context.tiposprogramas.Find(id);
                    if (entidad == null)
                    {
                    ...
                    {
                        entidadModelo.id = entidad.idtp;
                        entidadModelo.descripcion = entidad.descripciontp;
                        entidadModelo.sistema = entidad.sistematp;
                        entidadModelo.estado = entidad.estadotp;
        ...
        public static void Delete(string id)
        {
            if (!(string.IsNullOrEmpty(id)))
            {
                using (_context = new SGPTEntidades())
                {
                    string commandString = String.Format("DELETE FROM sgpt.tiposprograma WHERE idtp={0};", id);
        ...
        public static TipoProgramaModelo Find(string id)
        {
            var modelo = new TipoProgramaModelo();
            if (!(string.IsNullOrEmpty(id)))
            {
                using (_context = new SGPTEntidades())
                {
                    ...
                    {
                        modelo.id = entidad.idtp;
                        modelo.descripcion = entidad.descripciontp;
                        modelo.sistema = entidad.sistematp;
                        modelo.estado = entidad.estadotp;
        ...
        public static bool FindPK(string id, Boolean eliminado)
        {
            if (!(string.IsNullOrEmpty(id) || string.IsNullOrWhiteSpace(id)))
            {
                using (_context = new SGPTEntidades())
                {
                    ...
                    var entidad = _context.tiposprogramas
                            .Where(b => b.estadotp == "B")
                            .FirstOrDefault();
                    if (entidad == null)
                    {
                        return false;
        ...
        public static List<TipoProgramaModelo> GetAllPkContenido(string texto)
        {

            using (_context = new SGPTEntidades())
            {
                string Texto = texto.ToUpper();
                return _context.tiposprogramas.Select(entidad =>
                new TipoProgramaModelo
                {
                    id = entidad.idtp,
                    descripcion = entidad.descripciontp,
                    sistema = entidad.sistematp,
                    estado = entidad.estadotp
        ...
        public static void UpdateModelo(TipoProgramaModelo modelo)
        {
            if (!(modelo == null))
            {
                using (_context = new SGPTEntidades())
                {
                    tiposprograma entidad = _context.tiposprogramas.Find(modelo.id);
                    if (entidad == null)
                    {
                    ...
                    {
                        entidad.idtp = modelo.id;
                        entidad.descripciontp = modelo.descripcion;
                        entidad.sistematp = modelo.sistema;
                        entidad.estadotp = modelo.estado;
                    }
        ...
        public static bool UpdateModelo(TipoProgramaModelo modelo, Boolean actualizar)
        {
            if (!(modelo == null))
            {
                try
            ...
            {
                {
                    ...
                    try
                    {
                        using (_context = new SGPTEntidades())
                        {
                            string commandString = String.Format("UPDATE sgpt.tiposprograma SET estadotp = 'B' WHERE idtp={0};", id);
                            commandString = MetodosModelo.ordenConversionToSQL(commandString);
                            _context.Database.ExecuteSqlCommand(commandString);
                            _context.SaveChanges();
        ...
        public static List<TipoProgramaModelo> GetAll()
        {
            try
            {
                using (_context = new SGPTEntidades())
        ...
        public static bool FindTexto(string texto)
        {
            if (!((string.IsNullOrEmpty(texto) || string.IsNullOrWhiteSpace(texto))))
            {
                ...
                using (_context = new SGPTEntidades())
                {
                    var entidad = (from p in _context.tiposprogramas
                                   where p.descripciontp.ToLower().Equals(busqueda)

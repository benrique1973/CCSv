...
{
    public class WrapperSubPregunta : DependencyObject
    {
        public static readonly DependencyProperty listaEntidadPreguntaSeleccionProperty =
             DependencyProperty.Register("listaEntidadPreguntaSeleccion", typeof(ObservableCollection<DetalleCuestionarioModelo>),
             typeof(WrapperSubPregunta), new FrameworkPropertyMetadata());

        public ObservableCollection<DetalleCuestionarioModelo> listaEntidadPreguntaSeleccion
        {
            get { return (ObservableCollection<DetalleCuestionarioModelo>)GetValue(listaEntidadPreguntaSeleccionProperty); }
            set { SetValue(listaEntidadPreguntaSeleccionProperty, value); }
        }
    }
    public class ComboSeleccionPreguntaRule : ValidationRule
    {
        public WrapperSubPregunta Wrapper { get; set; }
        public override ValidationResult Validate(object value, CultureInfo cultureInfo)
        {
            try
            {
                ...
                if (eleccion != null)
                {
                    if (eleccion.id == 0)
                    {
                        return new ValidationResult(false, "Debe seleccionar el tipo de pregunta ");
                    }
                    else
                    {
                        ObservableCollection<DetalleCuestionarioModelo> castedCollection = this.Wrapper.listaEntidadPreguntaSeleccion;
